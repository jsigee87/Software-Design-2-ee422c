...
public class SortTools {
    ...
    public static void insertSort(int[] x, int n) {
    	...
    	for (; iterator < n; iterator ++) {
    		if (x[iterator] < target){
    			appended_list[iterator] = x[iterator];
    		}
    		if (x[iterator] == target) {
    			appended_list[iterator] = x[iterator];
    			break;
    		...
    		if (x[iterator] > target) {
    			appended_list[iterator] = target;
    			break;
    		}
    	}
    	
    	int new_iterator = iterator + 1;
    	for (; new_iterator < n + 1; new_iterator ++) {
    		appended_list[new_iterator] = x[iterator];
    	}
